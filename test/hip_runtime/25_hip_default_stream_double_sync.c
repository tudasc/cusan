...
// RUN: %tsan-options %cusan_test_dir/%basename_t.exe 2>&1 | %filecheck %s -DFILENAME=%s --check-prefixes=%sync-check
...
int main(int argc, char* argv[]) {
  ...
  write_kernel_delay<<<blocksPerGrid, threadsPerBlock, 0, stream1>>>(managed_data, size, 128, 545912);
