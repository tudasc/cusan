...
int main(int argc, char* argv[]) {
  ...
  int* dummy_d_data;
  size_t dummy_pitch;
  hipMallocPitch(&dummy_d_data, &dummy_pitch, width * sizeof(int), height);
  ...
  hipStream_t stream2;
  hipStreamCreateWithPriority(&stream2, hipStreamDefault, -1);
  ...
  hipMemset2D(dummy_d_data, dummy_pitch, 0, width, height);
#endif
